...
def get_sub_model(model_parameters, user_IDs, item_IDs, cate_IDs):
    if len(user_IDs) == 1 and len(item_IDs) == 1 and len(cate_IDs) == 1 and user_IDs[0] == 0 and item_IDs[0] == 0 and cate_IDs[0] == 0:
        return
    for i, model_parameter in enumerate(model_parameters):
        if i == 0:
            ...
            model_parameters[i] = model_parameter[cate_IDs]
        else:
            break
...
def choice_send_parameter(communication, received_dict, timestamp_queue, historical_model_dict, layer_name_queue, connection_socket):
    ...
    cate_IDs = received_dict['cate_IDs']
    ...
    get_sub_model(model_parameters, user_IDs, item_IDs, cate_IDs)
...
def delay_compensation(timestamp_queue, historical_model_dict, batches_info_dict, received_dict, delay_compensation_para):
    ...
    if model_timestamp == timestamp_queue[-1]:
        ...
        cate_IDs = batches_info_dict[client_ID]['cate_IDs']

        # get the sub model corresponding to the client's model
        get_sub_model(old_model_paras, user_IDs, item_IDs, cate_IDs)
        get_sub_model(current_model_paras, user_IDs, item_IDs, cate_IDs)
...
def gather_delta_model_parameter(clients, clients_message_list, clients_delta_model_list, clients_train_set_size_list,
                                 user_num, item_num, cate_num, trained_samples_num, size_avg_flag, sess):
    ...
    for i in range(len(clients)):
        ...
        cate_IDs = client_message['cate_IDs']

        train_set_size = clients_train_set_size_list[i]
        # aggregate ratio according to one client's training set size, if size_avg_flag
        if len(user_IDs) == 1 and len(item_IDs) == 1 and len(cate_IDs) == 1 and user_IDs[0] == 0 and item_IDs[0] == 0 and cate_IDs[0] == 0: # 手机客户端开发时MNN未支持端上模型动态调整，故特殊处理（ToDo）
            ...
            for index, delta_model_parameter in enumerate(client_delta_model):
                if index == 0:
                    ...
                    for client_index in range(len(delta_model_parameter)):
                        ps_index = client_index
                        gathered_delta_weights[index][ps_index] += delta_model_parameter[client_index] * train_set_size
                        cate_num[ps_index] += train_set_size
                else:
                    gathered_delta_weights[index] = gathered_delta_weights[index] + delta_model_parameter * train_set_size
        else:
            trained_samples_num[0] += train_set_size
            #user_IDs_appear_times = client_message['user_IDs_appear_times']
            #item_IDs_appear_times = client_message['item_IDs_appear_times']
            #cate_IDs_appear_times = client_message['cate_IDs_appear_times']
            ...
            for index, delta_model_parameter in enumerate(client_delta_model):
                if index == 0:
                    ...
                    for client_index in range(len(delta_model_parameter)):
                        ps_index = cate_IDs[client_index]
                        gathered_delta_weights[index][ps_index] += delta_model_parameter[client_index] * train_set_size
                        cate_num[ps_index] += train_set_size
                else:
    ...
    for index, delta_weights in enumerate(gathered_delta_weights):
        if index == 0:
            ...
            for cc in range(dataset_info['cate_count']):
                if cate_num[cc] > 0:
                    gathered_delta_weights[index][cc] = gathered_delta_weights[index][cc] / cate_num[cc]
                    new_weights[index][cc] = original_weights[index][cc] + gathered_delta_weights[index][cc]
                else:
                    new_weights[index][cc] = original_weights[index][cc]
        else:
...
def fedsubavg_do_weights_average(gathered_delta_weights, dataset_info, trained_samples_num, sess):
    ...
    cate_coef = np.loadtxt('coefs/cate_coef.txt', dtype=str, delimiter="\t")
    original_weights = sess.run(tf.trainable_variables())
    new_weights = [np.zeros(weights.shape) for weights in original_weights]
    for index, delta_weights in enumerate(gathered_delta_weights):
        if index == 0:
            ...
            for cc in range(dataset_info['cate_count']):
                if trained_samples_num[0] > 0:
                    gathered_delta_weights[index][cc] = float(cate_coef[cc]) * gathered_delta_weights[index][cc] / trained_samples_num[0]
                    new_weights[index][cc] = original_weights[index][cc] + gathered_delta_weights[index][cc]
                else:
                    new_weights[index][cc] = original_weights[index][cc]
        else:
...
def fedavg_do_weights_average(gathered_delta_weights, dataset_info, trained_samples_num, sess):
    ...
    for index, delta_weights in enumerate(gathered_delta_weights):
        if index == 0:
            ...
            for cc in range(dataset_info['cate_count']):
                if trained_samples_num[0] > 0:
                    gathered_delta_weights[index][cc] = gathered_delta_weights[index][cc] / trained_samples_num[0]
                    new_weights[index][cc] = original_weights[index][cc] + gathered_delta_weights[index][cc]
                else:
                    new_weights[index][cc] = original_weights[index][cc]
        else:
...
def create_csv(path):
    with open(path, 'w') as f:
        ...
        csv_head = ["round_num", "test_auc"]
        csv_writer.writerow(csv_head)

def write_csv(path, round_num, test_auc):
    with open(path, 'a+') as f:
        csv_writer = csv.writer(f)
        data_row = [round_num, test_auc]
...
def update_model(return_model_queue, return_model_queue_lock, gathered_weights_dict, valid_updates_queue,\
    ...
    test_data = DataIterator('./taobao_data_process/taobao_local_test_remap', hyperparameters['test_batch_size'])
    ...
    model = Model_DIN(user_count, item_count, cate_count, hyperparameters['embedding_dim'], hyperparameters['opt_alg'])
    ...
    print('-----------------------------------------------------------------------')
    print('Initialized model: test_auc: %.4f ---- test_loss: %f ---- accuracy: %f' % (test_auc, loss_sum, accuracy_sum))
    print('-----------------------------------------------------------------------')
    sys.stdout.flush()
    ...
    write_csv(path, round_num, test_auc)

    #-------------------------------------------- START: build the graph ---------------------------------------------
    #my_plot = Plot()
    #my_plot.add(0, test_auc)
    gui_evaluate_result.append([round_num, test_auc])
    gui_event.set()
        ...
        if len(valid_updates_queue) >= hyperparameters['sync_parameter']:
            ...
            with g1.as_default():
                ...
                gathered_delta_weights = gather_delta_model_parameter(valid_updates_queue, clients_message_list, clients_delta_model_list,
                                                                      clients_train_set_size_list, user_num, item_num, cate_num, trained_samples_num, size_avg_flag, sess)
                ...
                if round_num % 1 == 0:
                    ...
                    print(multiprocessing.current_process().name + 'Round %d Model Performance: test_auc: %.4f ---- test_loss: %f ---- accuracy: %f' %
