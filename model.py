...
from general_functions import din_attention
from Dice import dice, parametric_relu


class Model(object):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, opt_alg):
    ...
    def build_fcn_net(self, inp, use_dice=True):
        ...
        if use_dice:
            ...
            dnn1 = prelu(dnn1, 'parametric_relu_1')

        dnn2 = tf.layers.dense(dnn1, 80, activation=None, name='f2')    # [B, 80]
        if use_dice:
            dnn2 = dice(dnn2, name='dice_2')
        else:
            dnn2 = prelu(dnn2, 'parametric_relu_2')
        ...
        with tf.name_scope('Metrics'):
            ...
            if self.opt_alg == 'adam':
                ...
                clip_gradients, _ = tf.clip_by_global_norm(gradients, 0.1) # original din 5
...
class Model_DIN(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, opt_alg='sgd'):
        super(Model_DIN, self).__init__(n_uid, n_mid, n_cat, EMBEDDING_DIM, opt_alg)
